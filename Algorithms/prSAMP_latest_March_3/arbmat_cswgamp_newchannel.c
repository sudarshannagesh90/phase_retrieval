...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    double *ry, *rF, *cF;
    complex double *F;
    double *p_prms, *rinit, *cinit, t_max, par_frac, eps, damp_seq, damp_par, vnf;
    ...
    mwIndex *ir_, *jc_;
    mxArray *opt_tm, *opt_ep,*opt_del, *opt_prms, *opt_in, *opt_par_frac, *opt_da_seq, *opt_da_par,
    ...
    if (nrhs > 2) {
        ...
        opt_da_seq = mxGetField(prhs[2], 0, "damp_seq");
        opt_da_par = mxGetField(prhs[2], 0, "damp_par");
        opt_par_frac = mxGetField(prhs[2], 0, "par_frac");
        opt_di = mxGetField(prhs[2], 0, "display");
        opt_ou = mxGetField(prhs[2], 0, "output");
        opt_vn = mxGetField(prhs[2], 0, "vnf");

    } else {
        opt_tm = opt_ep = opt_del = opt_prms = opt_in = opt_da_seq = opt_da_par = opt_par_frac = opt_vn = opt_di = opt_ou = NULL;
    }
                     /* Prior */
    ...
    delta = malloc(sizeof(complex double) * 7);
    if (opt_del)
        delta = mxGetPr(opt_del);
    else {
		for (mu = 0; mu < m; mu++)
			delta[mu]=1e-8;
    }
    ...
    damp_seq = opt_da_seq ? *mxGetPr(opt_da_seq) : 0;
	damp_par = opt_da_par ? *mxGetPr(opt_da_par) : 0;
	par_frac = opt_par_frac ? *mxGetPr(opt_par_frac) : 1000;
    ...
    if (mxIsSparse(prhs[1])) {
        ...
        F = mxMalloc(sizeof(complex double) * nnz);//recently added complex
...
//         printf("c[1] is %.4e \n \n", c[1]);
        ...
        arbmat_cgamp_newchannel(n, m, y, F, ir, jc,
            delta, p_prms,
            t_max, par_frac, eps, damp_seq, damp_par, vnf, disp, output,
            a, c);
        
		// partial_cgamp(n, m, y, ir, jc,
            // delta, p_prms,
