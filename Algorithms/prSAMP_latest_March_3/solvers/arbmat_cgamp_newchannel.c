...
void arbmat_cgamp_newchannel (
    ...
    if(disp){
        printf("Code does not yet support complex measurement matrices\n");
        printf("F[0] is %f + i%f\n", creal(F[0]), cimag(F[0]));
        printf("delta[0]: %.2e; t_max: %3d; eps: %.2e; damp_par %.2e; vnf: %.2e\n", delta[0], t_max, eps, damp_par, vnf);
...
//         sig[i]=x_var_init;
    ...
    //t1 = time(NULL);
    for (t = 0; t < t_max; t++) {
		if ((par_frac>0||t==0)&&((t%par_frac)!=(par_frac-1))){//Perform sequential updates on all but 1/par_frac of the updates.  If parfrac is less than 0 perform parallel updates everywhere
			...
			for (i = 0; i < n; i++) {
				for (idx = jc[i]; idx < jc[i + 1]; idx++) {
					...
					c_proj[ ir[idx] ] += (cabs(F[idx]) * cabs(F[idx])) * c[i];
				}
			}
			...
			for (mu = 0; mu < m; mu++)
				channel(y[mu], o[mu], v[mu], delta[mu], &g[mu], &dg[mu], vnf);
			...
			for (key = 0; key < n; key++) {
				...
				for (idx = jc[i]; idx < jc[i + 1]; idx++) {
					...
					dg_proj -= (cabs(F[idx]) * cabs(F[idx])) * dg[ ir[idx] ];
				}
				if(t>0)
				...
				for (idx = jc[i]; idx < jc[i + 1]; idx++) {
					...
					v[mu] += (cabs(F[idx]) * cabs(F[idx])) * (c[i] - c_old);
...
*/
            ...
            for (i = 0; i < n; i++)
                a_norm+=pow(cabs(a[i]),2);
            a_norm=sqrt(a_norm);
            printf("Before Nonlinearity Norm of Estimate is %.4e \n", a_norm);
            ...
            for (i = 0; i < n; i++)
                a_norm+=pow(cabs(a[i]),2);
            a_norm=sqrt(a_norm);
            printf("After Nonlinearity Norm of Estimate is %.4e \n", a_norm);
...
/*		if(disp){
			printf("Code does not yet support complex measurement matrices\n");
			printf("F[0] is %f + i%f\n", creal(F[0]), cimag(F[0]));
			printf("F[2,6] is %f + i%f\n", creal(F[jc[6]+2]), cimag(F[jc[6]+2]));
			printf("delta[0]: %.2e; t_max: %3d; eps: %.2e; damp_par %.2e; vnf: %.2e\n", delta[0], t_max, eps, damp_par, vnf);
			printf("c[0]: %.2e; a[0]: %.2e\n", c[0], a[0]);
			printf("prmts: %.2e; %.2e; %.2e;\n", p_prms[0], p_prms[1], p_prms[2]);
		}
